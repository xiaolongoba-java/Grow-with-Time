import type { FilterState, Task, TaskPriority, TaskStatusFilter } from "@/types";
...
export function filterTasksByStatus(
  ...
  if (status === "active") return tasks.filter(isActiveTask);
  if (status === "completed") {
...
/**
 * The inbox contains all actionable unfinished work. A future occurrence that
 * was auto-generated by completing a repeating task stays dormant until its
 * due date, so completing a routine does not appear to resurrect the same row.
 */
export function isInboxTask(task: Task, today = todayDateString()): boolean {
  if (!isActiveTask(task) || Boolean(task.deleted_at)) return false;
  const dormantRepeat = Boolean(
    task.repeat_rule &&
    task.generated_from_id &&
    task.due_date &&
    task.due_date > today,
  );
  return !dormantRepeat;
}
...
export function filterTasksByView(
  ...
  switch (view) {
    ...
    case "myday":
      ...
      break;
    case "inbox":
      // The inbox is the full lifecycle queue: every unfinished root task,
      // regardless of whether it has already been scheduled for a date. Future
      // auto-generated repeat occurrences remain dormant until their due date.
