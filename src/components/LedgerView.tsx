...
export function LedgerView({ mode = "ledger" }: { mode?: "ledger" | "budget" }) {
  ...
  async function toggleHidden() { const next = !hidden; setHidden(next); await setSetting("ledger_hide_amount", String(next)); }
    ...
    <main className="main-workspace ledger-view">
      <header className="ledger-header">
        ...
        <div className="ledger-head-actions">
          ...
          <button className="btn-ghost" onClick={() => void toggleHidden()}>{hidden ? "显示金额" : "隐藏金额"}</button>
          <button className="btn-primary" onClick={() => openDrawer()}>＋ 记一笔</button>
        </div>
      </header>

      {mode === "budget" ? (
        <BudgetPanel month={month} expense={expense} budget={budget} hidden={hidden} onSaved={async () => { await load(); setNotice({ text: "预算已保存" }); }} />
      ) : (
        <>
          <section className="ledger-stat-grid" aria-label="本月收支摘要">
            <article><span>本月支出</span><strong className="expense">{formatLedgerMoney(expense, hidden)}</strong><small>{transactions.filter((item) => item.type === "expense").length} 笔支出</small></article>
            <article><span>本月收入</span><strong>{formatLedgerMoney(income, hidden)}</strong><small>{transactions.filter((item) => item.type === "income").length} 笔收入</small></article>
            <article><span>本月结余</span><strong>{formatLedgerMoney(income - expense, hidden)}</strong><small>{income - expense >= 0 ? "保持从容" : "支出高于收入"}</small></article>
            <article><span>预算进度</span><strong>{budget ? `${Math.round(budgetRatio * 100)}%` : "未设置"}</strong><div className="ledger-progress"><i style={{ width: `${Math.min(100, budgetRatio * 100)}%` }} /></div><small>{budget === 0 ? "可在预算页设置" : budgetRatio > 1 ? `已超支 ${formatLedgerMoney(expense - budget, hidden)}` : budgetRatio === 1 ? "本月预算已用完" : `还可用 ${formatLedgerMoney(budget - expense, hidden)}`}</small></article>
          </section>

          <section className="ledger-insight-grid">
            <article className="ledger-card"><div className="ledger-card-title"><div><h3>支出去向</h3><p>本月分类构成</p></div></div><div className="ledger-donut-wrap"><div className="ledger-donut" style={{ background: composition }}><span>{hidden ? "••••" : `${summary.length} 类`}</span></div><div className="ledger-legend">{summary.slice(0, 6).map((item) => <div key={item.category_id}><i style={{ background: item.color }} /><span>{item.name}</span><strong>{expense ? Math.round(item.amount_cents / expense * 100) : 0}%</strong></div>)}{!summary.length && <p>记下第一笔支出后，这里会出现清晰的构成。</p>}</div></div></article>
      ...
      {drawer && <div className="ledger-overlay" onMouseDown={(event) => { if (event.target === event.currentTarget) setDrawer(false); }}><section className="ledger-drawer" role="dialog" aria-modal="true" aria-labelledby="ledger-drawer-title"><header><div><p className="ledger-eyebrow">{editing ? "编辑记录" : "快速记账"}</p><h3 id="ledger-drawer-title">{editing ? "修改这一笔" : "记一笔"}</h3></div><button className="ledger-close" aria-label="关闭记账面板" onClick={() => setDrawer(false)}>×</button></header><div className="ledger-kind-tabs"><button className={draft.kind === "expense" ? "active" : ""} onClick={() => setKind("expense")}>支出</button><button className={draft.kind === "income" ? "active" : ""} onClick={() => setKind("income")}>收入</button></div><label className="ledger-amount"><span>金额</span><div><b>¥</b><input ref={amountRef} value={draft.amount} inputMode="decimal" autoComplete="off" placeholder="0.00" aria-describedby={error ? "ledger-error" : undefined} onChange={(event) => { const value = event.target.value.replace(/[^\d.]/g, ""); if (value.length <= 11) setDraft((current) => ({ ...current, amount: value })); }} onKeyDown={(event) => { if (event.key === "Enter") void save(); }} /></div></label><div className="ledger-quick-amounts">{[10, 20, 50, 100].map((value) => <button key={value} onClick={() => setDraft((current) => ({ ...current, amount: String(value) }))}>¥{value}</button>)}</div><fieldset><legend>分类</legend><div className="ledger-choice-grid">{activeCategories.map((item) => <button type="button" key={item.id} className={draft.categoryId === item.id ? "active" : ""} onClick={() => setDraft((value) => ({ ...value, categoryId: item.id }))}><i style={{ color: item.color }}>{ICONS[item.icon] ?? "·"}</i>{item.name}</button>)}</div></fieldset><div className="ledger-form-row"><label>日期<input type="date" value={draft.date} onChange={(event) => setDraft((value) => ({ ...value, date: event.target.value }))} /></label><label>账户<select value={draft.accountId} onChange={(event) => setDraft((value) => ({ ...value, accountId: Number(event.target.value) }))}>{activeAccounts.map((item) => <option key={item.id} value={item.id}>{accountGlyph(item.kind)} · {item.name}</option>)}</select></label></div><label>备注（可选）<input maxLength={60} value={draft.note} placeholder="例如：午餐、地铁…" onChange={(event) => setDraft((value) => ({ ...value, note: event.target.value }))} /></label>{error && <p className="ledger-error" id="ledger-error" role="alert">{error}</p>}<footer><button className="btn-ghost" onClick={() => setDrawer(false)}>取消</button><button className="btn-primary" onClick={() => void save()}>保存记录</button></footer></section></div>}
