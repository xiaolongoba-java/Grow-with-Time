...
import { activityLevel, calculateGoalProgress, longestDateStreak } from "@/lib/growth";
...
export function GrowthView() {
  ...
  const currentStreak = useMemo(() => {
    ...
    const date = new Date();
    for (;;) {
      if (!entriesByDate.has(toDateKey(date))) break;
      streak += 1;
  ...
  const totalValue = visibleEntries.reduce((sum, entry) => sum + Number(entry.value), 0);
  ...
  const submitGoal = async () => {
    ...
    await createGoal({
      ...
      current_value: Number(startValue) || 0,
          ...
          <section className="growth-panel heatmap-panel">
            ...
            <div className="growth-heatmap" aria-label="年度成长热点图">
              {days.map((date) => {
                ...
                const level = activityLevel(entriesByDate.get(key) ?? []);
        ...
        selectedGoal ? (
          <section className="goal-detail">
            ...
            <div className="goal-detail-hero" style={{ "--goal-color": selectedGoal.color } as CSSProperties}>
              <div><span>{selectedGoal.status === "paused" ? "已暂停" : selectedGoal.status === "completed" ? "已完成" : "进行中"}</span><h2>{selectedGoal.title}</h2><p>{selectedGoal.motivation || selectedGoal.description || "每一步都算数。"}</p></div>
            ...
            <div className="goal-detail-actions">
              <input type="number" value={entryValue} onChange={(event) => setEntryValue(event.target.value)} />
              <input value={entryNote} placeholder="记录今天的进展" onChange={(event) => setEntryNote(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void submitEntry(); }} />
              <button className="btn-primary" onClick={() => void submitEntry()}>记录 +{entryValue}{selectedGoal.unit}</button>
              <button className="btn-ghost" onClick={() => void updateGoal(selectedGoal.id, { status: selectedGoal.status === "paused" ? "active" : "paused" }).then(refresh)}>{selectedGoal.status === "paused" ? "继续目标" : "暂停"}</button>
            </div>
            <div className="growth-columns">
              ...
              <section className="growth-panel"><h3>成长时间线</h3>{entries.filter((entry) => entry.goal_id === selectedGoal.id).slice(0,20).map((entry) => <div key={entry.id} className="growth-entry"><time>{entry.entry_date}</time><div><strong>+{entry.value}{selectedGoal.unit}</strong><p>{entry.note || entry.source_type}</p></div></div>)}</section>
            </div>
          </section>
        ) : (
