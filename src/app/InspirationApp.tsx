...
export function InspirationApp() {
  ...
  return <main className="inspiration-shell"><header><div><span>拾念</span><strong>拾起一闪而过的灵感</strong></div><div className="inspiration-head-actions"><small>{shortcutLabel}</small><button type="button" className="page-close-btn" aria-label="关闭" onClick={() => void getCurrentWebviewWindow().hide()}>×</button></div></header><textarea ref={inputRef} autoFocus value={content} onChange={(event) => setContent(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); void save(); } }} placeholder="现在想到了什么？ #标签" /><footer><span>{saved ? "已拾起" : "Enter 保存 · Shift+Enter 换行"}</span><button onClick={() => void save()}>拾起</button></footer></main>;
}
