...
export function NotificationPopupApp() {
  ...
  if (!item) return null;
  const dismiss = async () => { await setNotificationStatus(item.id, "dismissed"); await emit("notification:state-changed", item.id); hide(); };
  const snooze = async () => { await snoozeNotification(item.id, 10); await emit("notification:state-changed", item.id); hide(); };
  const open = async () => {
    await setNotificationStatus(item.id, "read");
    if (item.task_id) await emit("notification:open-task", item.task_id);
    await emit("notification:state-changed", item.id);
