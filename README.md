...
本项目依照 [Apache License 2.0](LICENSE) 以“现状”提供。在中华人民共和国法律允许的最大范围内，项目作者及贡献者不对软件的适销性、特定用途适用性、持续可用性、完全无误或不侵权作出明示或默示保证。使用者应自行判断本软件是否适合其使用场景，并自行承担正常使用风险。

- 本软件是个人效率与记录工具，不构成医疗、法律、财务、投资或其他专业意见，也不应作为紧急事务、安全关键事务或法定期限管理的唯一依据。
- 提醒、倒计时、开机自启和系统通知可能受到操作系统权限、休眠、关机、系统策略、第三方运行时或设备状态影响。重要事项请同时采用其他可靠方式确认。
- 数据默认保存在使用者的本机。使用者应妥善保护设备、数据库、导出文件及 API Key，并定期创建和验证独立备份。因误操作、设备或磁盘故障、恶意软件、系统升级、权限变化或未及时备份造成的数据丢失风险，由使用者自行防范。
- AI 拆解、智能排期以及使用者自行配置的第三方 API 所生成的内容可能不准确、不完整或不适合具体场景。向第三方服务提交数据前，请使用者自行阅读其隐私政策和服务条款，并避免提交不必要的个人信息、敏感个人信息或机密内容。
- 使用者应确保其对导入、记录、处理和导出的内容拥有合法权利，并遵守适用的法律法规，不得利用本软件侵害他人合法权益或从事违法活动。
- 对于因使用或无法使用本软件产生的损失，责任承担以适用法律及 Apache License 2.0 的规定为准。本声明不排除或限制依法不得排除或限制的责任，包括因故意或重大过失造成对方财产损失以及造成对方人身损害的责任，也不影响消费者依法享有的权利。

本节是面向使用者的风险提示，不替代 Apache License 2.0，也不构成法律意见；如本节与适用的强制性法律规定冲突，以该法律规定为准。
...
#### v1.5.38 Windows 干净构建修复

- 将原生桌面宿主拆分为独立 Rust crate，消除首次构建时“编译宿主却要求宿主产物已存在”的循环依赖。
- 宿主使用独立锁文件，不再连带编译主应用的数据与通知插件；保留 v1.5.37 的原生桌面层、拖拽、IPC 和进程回收实现。
- Windows 安装包仍不下载或捆绑 WebView2；运行需要系统已安装 WebView2。
- 下载：[v1.5.38 Release](https://github.com/xiaolongoba-java/Grow-with-Time/releases/tag/v1.5.38)。
...
- 发布检查支持从全新检出运行：源码校验不再误依赖尚未生成的 `dist` 与 sidecar，实际打包阶段仍会严格生成并校验两者。
- Windows 原生宿主拆为独立轻量 Rust crate，避免 CI 为一个小型 sidecar 重复编译主应用的 SQLite、通知与插件全栈。
...
- 桌面仪表盘接入真实任务、习惯、倒计时、拾念与今日拾光数据；日历支持点击日期查看事项并完成任务。
- 仪表盘完成任务和习惯打卡统一走主应用业务流程，保留重复任务、成长贡献、Karma、项目进度与历史记录。
- 本地桌面桥接增加每次启动随机生成的会话令牌，未授权网页无法读取或修改个人数据。
- 收纳篮支持真实快捷方式图标、横向滚动和桌面底部居中；只有用户主动启用后才会在后续启动时恢复，不会在新安装时擅自整理桌面。
- 备忘录增加富文本/Markdown 格式、归档与恢复，并将新字段纳入正式 migration 和完整备份。
- 优化桌面仪表盘、日期弹窗、提醒页及窄屏布局，修复文字竖排、日历裁切和组件拥挤问题。
- 发布门禁通过 96 项单元测试、前端生产构建和 Rust 全目标编译检查。

### v1.5.34

| 平台 | 文件 | 下载 |
|------|------|------|
| macOS（Apple Silicon，M1/M2/M3/M4） | `.dmg` | [Grow.with.Time_1.5.34_aarch64.dmg](https://github.com/xiaolongoba-java/Grow-with-Time/releases/download/v1.5.34/Grow.with.Time_1.5.34_aarch64.dmg) |
| Windows x64 | `.exe` 安装包 | [Grow.with.Time_1.5.34_x64-setup.exe](https://github.com/xiaolongoba-java/Grow-with-Time/releases/download/v1.5.34/Grow.with.Time_1.5.34_x64-setup.exe) |

> 若链接暂不可用，请打开 [Releases](https://github.com/xiaolongoba-java/Grow-with-Time/releases/tag/v1.5.34) 或 Actions Artifacts 下载。
